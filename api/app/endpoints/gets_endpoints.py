...
from security import verify_token_user  # Importation de la fonction de vérification du token utilisateur
...
@router.get("/events", dependencies=[Depends(verify_token_user)])
...
@router.get("/events/search/", dependencies=[Depends(verify_token_user)])
def event_search(
...
@router.get("/events/artist/{artistName}", dependencies=[Depends(verify_token_user)])
def event_artist(
    artistName: str,
    page: int = 1
):
...
@router.get("/events/by-day-of-week", dependencies=[Depends(verify_token_user)])
...
@router.get("/events/by-venue", dependencies=[Depends(verify_token_user)])
...
@router.get("/events/streaming-vs-in-person", dependencies=[Depends(verify_token_user)])
...
@router.get("/events/is_popular", dependencies=[Depends(verify_token_user)])
def is_popular(
    artistName: Optional[str] = None,
    venueName: Optional[str] = None
): 
...
@router.get("/events/popular_event_these_days", dependencies=[Depends(verify_token_user)])
